...
async function seedLevels(dataSource) {
    const levelRepository = dataSource.getRepository(level_entity_1.Level);
    for (const levelData of exports.levelsSeedData) {
        const existingLevel = await levelRepository.findOne({
            where: { value: levelData.value },
        });
        if (existingLevel) {
            await levelRepository.update({ value: levelData.value }, {
                name: levelData.name,
...
//# sourceMappingURL=levels.seed.js.map